...
def plot_floorplan_bboxs(bboxes, corners=None, edges=None, scale=256):
    """Draw floorplan map where different colors indicate different rooms
    """
    colors = colors_12

    bboxes = [(box * scale / 256).round().astype(np.int) for box in bboxes]

    # define the color map
    wall_colors = [colors[i] for i in range(len(bboxes))]

    colorMap = [tuple(int(h[i:i + 2], 16) for i in (1, 3, 5)) for h in wall_colors]
    colorMap = np.asarray(colorMap)
    if len(bboxes) > 0:
        colorMap = np.concatenate([np.full(shape=(1, 3), fill_value=0), colorMap], axis=0).astype(
            np.uint8)
    ...
    alpha_channels = np.zeros(colorMap.shape[0], dtype=np.uint8)
    alpha_channels[1:len(bboxes) + 1] = 255
    ...
    for idx, box in enumerate(bboxes):
        ...
        cv2.rectangle(room_map, start_point, end_point, color=idx + 1, thickness=5)
