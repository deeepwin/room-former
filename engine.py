...
from util.plot_utils import plot_room_map, plot_score_map, plot_floorplan_with_regions, plot_semantic_rich_floorplan, plot_floorplan_bboxs
        ...
        if plot_gt:
            for i, gt_inst in enumerate(gt_instances):
                if not semantic_rich:
                    ...
                    # WMART - plot wall bounding box (this is only room bbox, not wall)
                    gt_bboxes = []
                    for j, bbox in enumerate(gt_inst.gt_boxes):
                        gt_bboxes.append(bbox.cpu().numpy())

                    gt_floorpln_wall_map = plot_floorplan_bboxs(gt_bboxes, scale=1000)
                    cv2.imwrite(os.path.join(output_dir, '{}_gt_bboxs.png'.format(scene_ids[i])), gt_floorpln_wall_map)
